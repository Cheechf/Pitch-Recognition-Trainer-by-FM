...
def result():
    ...
    if mode == "game":
        ...
        return redirect(url_for('custom_result'))
...
def custom_result():
    return render_template("custom_result.html",
        correct_pitch=session.get("correct_pitch"),
        correct_zone=session.get("correct_zone"),
        user_guess=session.get("user_guess"),
        user_decision=session.get("user_decision"),
        reaction_time=session.get("reaction_time"),
        hit_type=session.get("hit_type"),
        inning=session.get("inning"),
        outs=session.get("outs"),
        balls=session.get("balls"),
        strikes=session.get("strikes"),
